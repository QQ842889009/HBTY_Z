  ...
  methods: {
    init: function (id, systemic) {
      ...
      that.$nextTick(() => {
        ...
        if (id) {
          that.$http
            .post("plcdata/tems/role/searchById", { id: id })
            .then((res) => {
        ...
        that.$http
          .get("plcdata/tems/permission/searchAllPermission")
          .then((res) => {
            console.log("所有的部门", res);
            let temp = [];
            for (let one of res.list) {
              let disabled = false;
              if (that.systemic) {
            ...
            console.log("所有权限", that.permisionList);
          });
        ///-----
        // that.$http(
        //   "permission/searchAllPermission",
        //   "GET",
        //   null,
        //   true,
        //   function (resp) {
    ...
    dataFormSubmit: function () {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          that.$http
            .post(
              `plcdata/tems/role/${!that.dataForm.id ? "insert" : "update"}`,
              that.dataForm
            )
            .then((res) => {
              if (res.rows == 1) {
          ...
          ////----
          // that.$http(
          //   `role/${!that.dataForm.id ? "insert" : "update"}`,
          //   "POST",
          //   that.dataForm,
          //   true,
          //   function (resp) {
